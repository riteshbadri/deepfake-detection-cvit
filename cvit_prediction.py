...
def plot_loss(train_loss, val_loss):
    epochs = range(1, len(train_loss) + 1)  # Create epoch indices
...
def plot_roc_curve(true_labels, predictions):
    ...
    roc_auc = auc(fpr, tpr)  # Calculate the Area Under Curve (AUC)

    # Plot the ROC curve
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, color='blue', lw=2, label=f"ROC Curve (AUC = {roc_auc:.2f})")
    plt.plot([0, 1], [0, 1], color='grey', lw=2, linestyle='--')  # Random classifier line
...
def test(model, dataloaders, dataset_sizes):
    ...
    for inputs, labels in dataloaders['test']:
        ...
        predictions.extend(output[:, 1].detach().cpu().numpy())  # Probability of "Fake" class
